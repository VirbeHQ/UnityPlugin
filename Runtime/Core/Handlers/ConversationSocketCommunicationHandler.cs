...
namespace Virbe.Core.Handlers
{
    internal sealed class ConversationSocketCommunicationHandler : ICommunicationHandler
    {
        ...
        private const string SocketError = "error";

        bool ICommunicationHandler.Initialized => _initialized;

        public readonly List<RequestActionType> DefinedActions = new() 
        { RequestActionType.SendAudioStream,
            RequestActionType.SendText, 
            RequestActionType.SendSignal };
        ...
        {
            return DefinedActions.Any(e => e == type);
        }
        ...
        Task ICommunicationHandler.MakeAction(RequestActionType type, params object[] args)
        {
            ...
            else if(type == RequestActionType.SendSignal)
            {
                SendSignal(args[0] as string, args[1] as string).Forget();
            }
            return Task.CompletedTask;
        }
        ...
        private async UniTaskVoid SendText(string text)
        {
            ...
            message.endUserId = _currentSession.UserId;
        ...
        private async UniTaskVoid SendSignal(string signalName, string signalValue)
        {
            if (_socketClient == null)
            {
                _logger.LogError($"[VIRBE] Socket not created, could not send speech chunk");
            ...
            message.endUserId = _currentSession.UserId;
            message.action = new MessageAction();
            message.action.Signal = new Signal();
            message.action.Signal.Name = signalName;
            message.action.Signal.Value = signalValue;
            await _socketClient.EmitAsync(ConversationMessage, message);
        }
        ...
        private Task ConnectToSocket()
        {
            ...
            _socketClient.On(SocketError, (response) => _logger.Log($"Socket error: {response.ToString()}"));
        ...
        private void OnConversationInitialize(string responseJson)
        {
            ...
            _actionToken.ConversationInitialized?.Invoke(_currentSession.ConversationId);
        }
        ...
        private async Task InitializeConversation(string conversationID)
        {
            ...
            if (!string.IsNullOrEmpty(conversationID))
            {
                ...
                msg.messagesSince = _lastMessageTime.ToString("yyyy-MM-ddTHH:mm:ss.fffZ", CultureInfo.InvariantCulture);
                _logger.Log($"Restoring conversation for ID: {conversationID} and time :{msg.messagesSince}");
            }
        ...
        private void ProcessResponse(Guid guid, VoiceData voiceData)
        {
            ...
            while (_speechChunks.TryPeek(out var firstChunk))
            {
                ...
                if (_speechChunks.TryDequeue(out var result))
                {
                    ...
                    {
                        cards.Add( new Card() { Title = cardItem.Title, Payload = cardItem.Payload.Value, PayloadType = cardItem.PayloadType });
                    }
